...
class StatsAdminOverview extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        for ($date = $startDate; $date->lte($endDate); $date->addDay()) {
            $customPackageCounts[] = CustomPackage::whereDate('created_at', $date->format('Y-m-d'))->count();
            $customerCounts[] = Customers::whereDate('created_at', $date->format('Y-m-d'))->count();
            $categoryCounts[] = Category::whereDate('created_at', $date->format('Y-m-d'))->count();
            $addonCounts[] = Addon::whereDate('created_at', $date->format('Y-m-d'))->count();
            $iternityTemplateCounts[] = IternityTemplate::whereDate('created_at', $date->format('Y-m-d'))->count();
            $packageTemplateCounts[] = PackageTemplate::whereDate('created_at', $date->format('Y-m-d'))->count();
            $destinationCounts[] = destination::whereDate('created_at', $date->format('Y-m-d'))->count();
            $hotelCategoryCounts[] = HotelCategory::whereDate('created_at', $date->format('Y-m-d'))->count();
            $hotelCounts[] = Hotel::whereDate('created_at', $date->format('Y-m-d'))->count();
            $roomCategoryCounts[] = RoomCategory::whereDate('created_at', $date->format('Y-m-d'))->count();
            $userCounts[] = User::whereDate('created_at', $date->format('Y-m-d'))->count(); // Count new users per day
            $revenuePerDay[] = Payment::whereDate('created_at', $date->format('Y-m-d'))->sum('amount_paid'); // Sum revenue per day
        }
        ...
        $stats = [
            ...
            Stat::make('No. of Customers', Customers::all()->count())
            ...
            Stat::make('Total Itinerary Template', IternityTemplate::all()->count())
            ...
            Stat::make('Total Destinations', destination::all()->count())
            ...
            Stat::make('Total Hotel Types', HotelCategory::all()->count())
            ...
            Stat::make('Total Hotels', Hotel::all()->count())
                ->chart($hotelCounts)
            ...
            Stat::make('Total Room Types', RoomCategory::all()->count())
        ...
        if ($userId === 1) {
            ...
            $stats[] = Stat::make('Total Users', User::count())
                ...
                ->description(array_sum($userCounts) . ' new users in 7 days')
            ...
            $stats[] = Stat::make('No. of Payment Voucher', Payment::where('user_id', $userId)->count())
                ...
                ->description(array_sum($paymentCounts) . ' payments in 7 days')
            ...
            $stats[] = Stat::make('Total Revenue', '₹ ' . Payment::where('user_id', $userId)->sum('amount_paid'))
                ...
                ->description('₹ ' . array_sum($revenuePerDay) . ' revenue generated in 7 days')
